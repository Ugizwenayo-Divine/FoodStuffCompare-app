...
export default class AuthenticationController {
  ...
  static userLogin = async (req, res) => {
    ...
    try{
      ...
      await createToken(token);
      successResponse(res, ok, customMessages.loginSuccess, token, loginUser);
    }
    catch(err){
      console.log(err);
  ...
  static findEmail = async (req, res) => {
    ...
    try{
    const userExist = await findByEmail(email.toLowerCase());
    if (userExist) {
        return responseHandlers.successResponse(res,ok,'all users',null,userExist);
    }
    return responseHandlers.errorResponse(res, badRequest, 'User does not exist');
    }
    catch(err){
      errorResponse(res, badRequest, err.message);
    }
